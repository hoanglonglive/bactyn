export default function StoreDetailLoading() {
  return (
    <div className="min-h-dvh bg-surface animate-pulse">
      {/* Header Skeleton */}
      <header className="sticky top-0 z-30 glass border-b border-border-subtle p-4 space-y-3">
        <div className="flex items-center gap-3">
          ...
          <div className="w-20 h-8 rounded-xl bg-indigo-500/10 border border-indigo-500/20" />
        </div>

        {/* Search Bar Skeleton */}
        <div className="flex items-center gap-2">
          <div className="h-9 flex-1 bg-surface-elevated rounded-xl border border-border-subtle" />
          <div className="h-9 w-16 bg-surface-elevated rounded-xl border border-border-subtle" />
        </div>
      ...
      <main className="px-3 py-4 max-w-6xl mx-auto">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2.5 sm:gap-3">
          {Array.from({ length: 10 }).map((_, i) => (
            <div
              key={i}
              className="rounded-2xl bg-surface-elevated border border-white/5 overflow-hidden aspect-[3/4] relative"
            >
