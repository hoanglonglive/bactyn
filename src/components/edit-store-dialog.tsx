...
import { X, ImagePlus, Loader2, Edit3, Trash2 } from "lucide-react";
...
export function EditStoreDialog({ store, onClose, onUpdated }: Props) {
  const router = useRouter();
  const [name, setName] = useState(store.name);
  const [note, setNote] = useState(store.note || "");
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [coverPreview, setCoverPreview] = useState<string | null>(store.cover_url || null);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (result.error) {
      ...
      if (onUpdated && result.data) {
        onUpdated(result.data as Store);
      }
      router.refresh();
      onClose();
    }
        ...
        <div className="flex items-center justify-between mb-5 pb-3 border-b border-white/5">
          <div>
            <h2 className="text-base font-extrabold text-white flex items-center gap-2">
              ...
              Sửa thông tin album gian hàng
            </h2>
            <p className="text-[11px] text-white/50 mt-0.5">
              Cập nhật tên, ghi chú và ảnh đại diện đại diện gian hàng
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label className="block text-xs font-semibold text-white/70">
                Ảnh đại diện / Ảnh bìa gian hàng
              </label>
              {coverPreview && (
                <button
                  type="button"
              ...
              {coverPreview ? (
                <>
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <span className="px-3 py-1.5 rounded-xl bg-amber-400 text-black font-extrabold text-xs shadow-lg">
                      Thay đổi ảnh
                    </span>
                  </div>
                </>
